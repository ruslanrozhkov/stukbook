class User < ApplicationRecord
  ...
  def friendship_status(user_2)
    friendship = Friendship.where(user_id: [self, user_2.id], friend_id: [self.id, user_2.id])
  ...
  def friendship_relation(user_2)
    Friendship.where(user_id: [self, user_2.id], friend_id: [self.id, user_2.id]).first
  end
end
